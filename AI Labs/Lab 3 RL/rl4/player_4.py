...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        if np.random.random() < epsilon: #Explore, if random<epsilon
            #np.random.random()=[0.0, 1.0], the probability that np.random gives a value lower than epsilon is epsilon
            action = np.random.choice(4)
        else: #Exploit, if random>epsilon, picking the highest Q-value
        ...
        sl_object = ScheduleLinear(anneal_timesteps, epsilon_final, epsilon_initial)
        epsilon_t = sl_object.value(current_total_steps) 
        # sl method value returns the current epsilon dependent on what timestep we're in
        # Initially epsilon will be bigger so that we explore more  
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff >= self.threshold:
            ...
            while not end_episode:
                ...
                #From assignment 2: action = np.nanargmax(Q[s_current])
                action = epsilon_greedy(Q, 
                                        ...
                                        self.epsilon_final, 
                                        self.annealing_timesteps)
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
                """
                Answer: observe several good results (<100 steps) in a row, followed by several 
                bad results (100 steps) in a row. Many episodes take the maximum steps, suggesting 
                the agent struggles to complete the task efficiently or converges to the goal.
                The Diff value is constant which means no significant improvement in the Q-table values.
                """
                ...
                best_next_action = np.nanmax(Q[s_next])  #Find the max Q-value for the next state
                Q[s_current, action] = Q[s_current, action] + lr * ( #lr=0.1
                    R + discount * best_next_action - Q[s_current, action] #discount=0.1 (gamma)
                )
            ...
            diff = np.nanmean(np.abs(Q - Q_old)) #Computes mean difference of the current Q-value table and the previous Q
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
            print(
...
class PlayerControllerRandom(PlayerController):
    ...
    def random_agent(self):
        ...
        moves = np.zeros((ns, na))
        policy = np.argmax(moves, axis=1) #the policy is the most profitable value
